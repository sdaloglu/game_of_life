...
int main(int argc, char* argv[]){
    ...
    */
    ...
    if (grid_size % process_sqrt != 0){
        std::cerr << "The size of the grid must be divisible by the square root of the number of processes" << std::endl;
        return 1;
    }
    ...
    // Reoranize the grid so that each chunk to be scattered is contiguous in memory
    ...
    MPI_Scatter(grid.getGrid(), chunk_size*chunk_size, MPI_INT, 
                local_grid.getGrid(), chunk_size*chunk_size, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    MPI_Gather(local_grid.getGrid(), chunk_size*chunk_size, MPI_INT, 
                grid.getGrid(), chunk_size*chunk_size, MPI_INT, 0, MPI_COMM_WORLD);
